...
# (6) Повторно запускаем envsubst т.к. в значениях переменных могут быть на другие переменные.
# (7) Забрасывает результат в файл, который подключается в nginx-конфиге.
cat /app/$ENV_PATH \
    | sed -e 's/\s*#.*$//' -e '/^\s*$/d' \
    | awk -F "=" '{print $1}' \
    | awk '{$1=$1};1' \
